    ...
    if (!csrf_verify($_POST['csrf_token'] ?? '')) {
        ...
        if (!$errors) {
            try {
                ...
                set_flash('message', 'Product added.');
...
<head>
<meta charset="utf-8">
<title><?= e(SITE_NAME) ?> - Add Product</title>
...
<body class="bg-lightbg min-h-screen">
  ...
  <main class="max-w-3xl mx-auto px-4 pt-24 pb-10">
    <div class="card">
      <h1 class="font-heading text-xl mb-4">Add Product</h1>
      ...
      <form method="post" class="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div>
          ...
          <input type="number" name="min_stock" class="input" required>
          <?php if (!empty($errors['min_stock'])): ?><div class="error"><?= e($errors['min_stock']) ?></div><?php endif; ?>
        </div>
        <div class="md:col-span-2">
          <label class="label">Composition</label>
          <textarea name="composition" class="input" rows="4" required></textarea>
